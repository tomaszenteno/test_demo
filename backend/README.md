...
- `GET /api/v1/students`
  - Returns a filtered list of students (optional filters: name, className, section, roll).

- `POST /api/v1/students`
  - Creates a new student.
  - Uses PostgreSQL function `student_add_update(jsonb)` to insert into `users` + `user_profiles`.
  - On success attempts to send verification email; if that fails, still returns a success message.

- `GET /api/v1/students/:id`
  - Returns detailed student profile (basic info + profile fields + reporter name).

- `PUT /api/v1/students/:id`
  - Updates student basic and profile details via `student_add_update(jsonb)`.

- `POST /api/v1/students/:id/status`
  - Updates `is_active` and status review fields in `users`.

- `DELETE /api/v1/students/:id` *(si lo implementaste)*
  - Soft/hard delete description según lo que hayas hecho.

### Notes

- Authentication:
  - JWT access token is stored in `accessToken` cookie.
  - CSRF protection enabled via `x-csrf-token` header + HMAC in JWT (`csrf_hmac`).
- Database:
  - Student logic uses `student_add_update(data jsonb)` in `seed_db/tables.sql`.
  - `class_name` and `section_name` in `user_profiles` reference `classes(name)` and `sections(name)`.

Postman collection for testing student CRUD is available at `postman/student-crud.postman_collection.json`.
